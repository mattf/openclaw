...
function serveDirectoryListing(
  ...
  const formatDate = (d: Date): string => d.toISOString().replace("T", " ").slice(0, 19);
  ...
  for (const f of files) {
    const fileUrl = browsePrefix + "/" + [...segments, f.name].map(encodeURIComponent).join("/");
    let size = "—";
    let mtime = "—";
    try {
  ...
  const uploadTokenParam = bearerToken ? `data-token="${h(bearerToken)}"` : "";
