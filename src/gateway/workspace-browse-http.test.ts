...
function makeUploadReq(url: string, boundary: string, body: Buffer): IncomingMessage {
...
describe("redirects", () => {
  ...
  it("301 redirects directory without trailing slash", async () => {
    ...
    const { res } = makeMockHttpResponse();
    await handleWorkspaceBrowseRequest(makeReq("/workspace/subdir"), res, makeOpts(tmpDir));
    expect(res.statusCode).toBe(301);
...
describe("file serving", () => {
  ...
  it("returns 404 for missing file", async () => {
    const { res } = makeMockHttpResponse();
    await handleWorkspaceBrowseRequest(makeReq("/workspace/missing.txt"), res, makeOpts(tmpDir));
  ...
  it("serves file in subdirectory", async () => {
    ...
    const { res, end } = makeMockHttpResponse();
    await handleWorkspaceBrowseRequest(makeReq("/workspace/sub/nested.txt"), res, makeOpts(tmpDir));
    expect(res.statusCode).toBe(200);
...
describe("path traversal", () => {
  it("rejects ..%2F traversal (403 or 404)", async () => {
    const { res } = makeMockHttpResponse();
    await handleWorkspaceBrowseRequest(makeReq("/workspace/..%2Fevil"), res, makeOpts(tmpDir));
    expect([403, 404]).toContain(res.statusCode);
  });

  it("rejects %2E%2E%2F traversal (403 or 404)", async () => {
    const { res } = makeMockHttpResponse();
    await handleWorkspaceBrowseRequest(makeReq("/workspace/%2E%2E%2Fevil"), res, makeOpts(tmpDir));
    expect([403, 404]).toContain(res.statusCode);
  });
});
