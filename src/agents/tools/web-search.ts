...
async function runWebSearch(params: {
  ...
  const cacheKey = normalizeCacheKey(
    params.provider === "brave" && effectiveBraveMode === "llm-context"
      ? `${params.provider}:llm-context:${params.braveBaseUrl ?? BRAVE_DEFAULT_BASE_URL}:${params.query}:${params.country || "default"}:${params.search_lang || params.language || "default"}:${params.freshness || "default"}`
      : `${params.provider}:${effectiveBraveMode}:${params.braveBaseUrl ?? BRAVE_DEFAULT_BASE_URL}:${params.query}:${params.count}:${params.country || "default"}:${params.search_lang || params.language || "default"}:${params.ui_lang || "default"}:${params.freshness || "default"}:${params.dateAfter || "default"}:${params.dateBefore || "default"}:${params.searchDomainFilter?.join(",") || "default"}:${params.maxTokens || "default"}:${params.maxTokensPerPage || "default"}:${providerSpecificKey}`,
  );
  ...
  if (effectiveBraveMode === "llm-context") {
    const { results: llmResults, sources } = await runBraveLlmContextSearch({
      ...
      apiKey: params.apiKey,
      timeoutSeconds: params.timeoutSeconds,
      baseUrl: params.braveBaseUrl ?? BRAVE_DEFAULT_BASE_URL,
      country: params.country,
  ...
  const url = new URL(`${params.braveBaseUrl ?? BRAVE_DEFAULT_BASE_URL}/web/search`);
  url.searchParams.set("q", params.query);
