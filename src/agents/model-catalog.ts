    ...
    try {
      ...
      for (const row of hookResults) {
        ...
        if (Array.isArray(row.input) && row.input.length > 0) {
          const validInput: ModelInputType[] = row.input.filter((v): v is ModelInputType =>
...
async function invokePluginManifestModelCatalog(plugin: {
  id: string;
  source: string;
  rootDir: string;
}): Promise<
...
> {
  ...
  const result = (await hook.call(entry)) as {
    entries?: Array<{
      id: string;
      name: string;
      provider: string;
